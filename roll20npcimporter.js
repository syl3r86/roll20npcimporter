...
class Roll20NpcImporter extends Application {
    ...
    hookActorList() {
        Hooks.on('renderActorDirectory', (app, html, data) => {
            const importButton = $('<button class="roll20-npc-import-list-btn" style="min-width: 96%; margin-top: 5px;"><i class="fas fa-file-import"></i> NPC Import</button>');
    ...
    async importNpc(sheetData, targetMode, compendiumName = '') {
        ...
        let actorList = game.actors._source;
        let actorCompendium;
        ...
        if (targetMode == 'compendium') {
            if (actorCompendium == null || actorCompendium == undefined) {
                ...
                if (conflictResult.conflict && conflictResult.choice === 'update') {
                    ...
                    // make sure to use the original art if the user chose so
                    if (conflictResult.ignoreImg === true) {
                        let originalActor = await actorCompendium.getEntity(conflictResult.originalId);
                        tmpActor.data.img = originalActor.data.img;
                        tmpActor.data.token.img = originalActor.data.token.img;
                    }
                    actorCompendium.updateEntity(tmpActor.data);
                } else {
                    console.log(`NPCImporter | ${npcData.name} will be created`);
